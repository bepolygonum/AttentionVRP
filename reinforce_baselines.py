...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            self.dataset = CVRPTW.make_dataset(
        ...
        self.bl_vals = rollout(self.model, self.dataset, self.opts).numpy()
    ...
    def epoch_callback(self, model, epoch):
        ...
        print("candidate_vals:", candidate_vals)

        candidate_mean = candidate_vals.mean()

        print("Epoch {} candidate mean {}, baseline epoch {} mean {}, difference {}".format(
            epoch, candidate_mean, self.epoch, self.mean, candidate_mean - self.mean))
        # greedy rollout as baseline, 若L(pi)-b(s)<0则更新
        if candidate_mean - self.mean < 0:
            # Calc p value
            print("candidate_vals:{} self.bl_vals:{}".format(candidate_vals,self.bl_vals))
            t, p = ttest_rel(candidate_vals, self.bl_vals)  # t检验
            print("t:{} p:{} candidate_vals:{} self.bl_vals:{}".format(t, p,candidate_vals,self.bl_vals))

            p_val = p / 2  # one-sided
            assert t < 0, "T-statistic should be negative"
            print("p-value: {}".format(p_val))
            if p_val < self.opts.bl_alpha:  # 仅在提升明显时更新baseline（p-value<0.05时差异显著，认为有明显提升）
